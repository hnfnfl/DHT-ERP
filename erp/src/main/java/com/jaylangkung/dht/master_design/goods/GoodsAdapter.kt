...
import com.jaylangkung.dht.databinding.ItemGoodsBinding
import com.jaylangkung.dht.master_design.AdditionalAdapter
import com.jaylangkung.dht.master_design.AdditionalEntity

class GoodsAdapter : RecyclerView.Adapter<GoodsAdapter.ItemHolder>() {

    private var list = ArrayList<GoodsEntity>()

    fun setItem(item: List<GoodsEntity>?) {
        if (item == null) return
        list.clear()
        list.addAll(item)
    ...
    class ItemHolder(private val binding: ItemGoodsBinding) : RecyclerView.ViewHolder(binding.root) {
        private lateinit var additionalAdapter: AdditionalAdapter
        private var listData: ArrayList<AdditionalEntity> = arrayListOf()

        fun bind(item: GoodsEntity) {
            additionalAdapter = AdditionalAdapter()
            with(binding) {
                tvGoodsCode.text = item.kode
                tvGoodsNameType.text = itemView.context.getString(
                    R.string.product_name_category, item.barang, item.jenis
                )
                tvGoodsStock.text = item.stok
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ItemHolder {
        val itemBinding = ItemGoodsBinding.inflate(LayoutInflater.from(parent.context), parent, false)
